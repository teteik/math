...
public class CubicSolver {
    ...
    static List<Double> solveCubic(double a, double b, double c) {
        ...
        if (D <= 0) {
            Interval interval = findRootOnInfiniteInterval(a, b, c, 0);
            roots.add(bisect(a, b, c, interval.left, interval.right));
        } else {
            double sqrtD = Math.sqrt(D);
            double alpha = (-2 * a - sqrtD) / 6;
            double beta = (-2 * a + sqrtD) / 6;

            double fAlpha = f(alpha, a, b, c);
            double fBeta = f(beta, a, b, c);

            if (fAlpha > eps && fBeta < -eps) {
                roots.add(bisect(a, b, c, findFiniteIntervalLeft(a, b, c, alpha).left, alpha));
                roots.add(bisect(a, b, c, alpha, beta));
                roots.add(bisect(a, b, c, beta, findFiniteIntervalRight(a, b, c, beta).right));
            } else if (fAlpha > eps && fBeta > eps) {
                Interval interval = findFiniteIntervalLeft(a, b, c, alpha);
                roots.add(bisect(a, b, c, interval.left, interval.right));
            } else if (fAlpha < -eps && fBeta < -eps) {
                Interval interval = findFiniteIntervalRight(a, b, c, beta);
                roots.add(bisect(a, b, c, interval.left, interval.right));
    ...
    static Interval findRootOnInfiniteInterval(double a, double b, double c, double x0) {
        double f0 = f(x0, a, b, c);
        if (Math.abs(f0) < eps) {
            return new Interval(x0, x0);
        } else if (f0 < -eps) {
            return findFiniteIntervalRight(a, b, c, x0);
        } else {
            return findFiniteIntervalLeft(a, b, c, x0);
        }
    }

    static double bisect(double a, double b, double c, double left, double right) {
        if (Math.abs(f(left, a, b, c)) < eps) return left;
        if (Math.abs(f(right, a, b, c)) < eps) return right;

        while (right - left > eps) {
            double mid = (left + right) / 2.0;
            double fMid = f(mid, a, b, c);
            if (Math.abs(fMid) < eps) return mid;
            if (f(left, a, b, c) * fMid < 0) {
                right = mid;
            } else {
                left = mid;
            }
